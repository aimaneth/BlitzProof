...
router.get('/:tokenId', async (req, res) => {
  try {
    ...
    if (refresh === 'true') {
      ...
      const freshData = await priceService.refreshTokenPrice(tokenId);
      res.json({
        success: true,
        data: freshData,
      ...
      const cachedData = await priceService.getCachedPrice(tokenId);
  ...
  try {
    ...
    for (let i = 0; i < tokenIds.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (tokenId: string) => {
        try {
          const data = await priceService.refreshTokenPrice(tokenId);
